                ...
                <tbody>
                    @foreach ($prodi as $prodi)
                        <tr>
                            ...
                            <td>{{ $prodi->gelar }}</td>
                            <td>
                                <a href="{{ url('admin/prodi/' . $prodi->id) }}" class="btn btn-success btn-sm"><i
                                        class="bi bi-eye"></i></a>
                                <a href="{{ url('admin/prodi/' . $prodi->id . '/edit') }}" class="btn btn-warning btn-sm"><i
                                        class="bi bi-pencil-square"></i></a>
                                <form onsubmit="return confirm('yakin menghapus data ??')" class='d-inline'
                                    action=" {{ url('admin/prodi/' . $prodi->id) }}" method="post">
                                    @csrf
                                    @method('DELETE')
